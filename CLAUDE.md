...
9. **Comments** - Only add comments that explain *why*, not *what*. Never write comments that restate the code (e.g., `// Cache the result` before a cache assignment, `// Check cache first` before an if statement). Let clear code speak for itself.
