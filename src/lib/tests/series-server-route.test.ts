...
/**
 * Tests for /api/series/slug/[slug] route
 */
describe('Series By Slug Route', () => {
	// Mock the SeriesService for slug route
	const mockGetSeriesBySlug = vi.fn();
	...
	it('should return series for valid slug', async () => {
		// Mock the SeriesService
		vi.doMock('$lib/server/services/series-service', () => {
			return {
				SeriesService: class {
					getSeriesBySlug = mockGetSeriesBySlug;
				}
			};
		});
		...
		await fc.assert(
			fc.asyncProperty(
				...
				async (slug, series: Series) => {
					mockGetSeriesBySlug.mockReset();
					mockGetSeriesBySlug.mockResolvedValue(series);
	...
	it('should return 400 for empty slug', async () => {
		...
		expect(response.status).toBe(400);
		expect(mockGetSeriesBySlug).not.toHaveBeenCalled();

		const responseText = await response.text();
		const errorData = JSON.parse(responseText);
		expect(errorData.message).toContain('slug');
	});

	it('should log request and response with duration', async () => {
		const { Logger } = await import('$lib/server/utils/logger');
		const mockLoggerInfo = vi.spyOn(Logger.prototype, 'info');
		...
		await fc.assert(
			fc.asyncProperty(
				...
				async (slug, series: Series) => {
					...
					await GET_SLUG(mockEvent);

					// Verify logging occurred
					expect(mockLoggerInfo).toHaveBeenCalled();

					// Find the "Fetching series by slug" log call
					const fetchingLogCall = mockLoggerInfo.mock.calls.find(
						(call) => call[0] === 'Fetching series by slug'
					);
					expect(fetchingLogCall).toBeDefined();
					if (fetchingLogCall) {
						expect(fetchingLogCall[1]).toHaveProperty('slug');
					}
					...
					expect(successLogCall).toBeDefined();
					if (successLogCall) {
						expect(successLogCall[1]).toHaveProperty('duration');
						expect(typeof successLogCall[1]?.duration).toBe('number');
					}
				}
			),
			{ numRuns: 100 }
