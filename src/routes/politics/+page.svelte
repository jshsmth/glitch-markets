<script lang="ts">
	...
	let tag_slug = $derived(selectedTag || 'politics');
	...
	const subcategoriesQuery = createQuery(() => ({
		queryKey: ['tags', 'politics', 'related'],
		queryFn: async () => {
			const res = await fetch('/api/tags/slug/politics/related');
	...
	const eventsQuery = createInfiniteQuery(() => ({
		queryKey: ['events', 'politics', tag_slug, status, sort],
