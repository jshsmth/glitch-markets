<script lang="ts">
	...
	let tag_slug = $derived(selectedTag || 'tech');
	...
	const subcategoriesQuery = createQuery(() => ({
		queryKey: ['tags', 'tech', 'related'],
		queryFn: async () => {
			const res = await fetch('/api/tags/slug/tech/related');
	...
	const eventsQuery = createInfiniteQuery(() => ({
		queryKey: ['events', 'tech', tag_slug, status, sort],
